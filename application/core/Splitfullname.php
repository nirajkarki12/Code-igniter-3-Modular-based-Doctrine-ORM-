...
class Splitfullname{
	...
	public static function split($full_name){

	$splitfullname = new Splitfullname();
	$name = $splitfullname->split_full_name($full_name);
	$explode = explode(" ", trim($name['fname']));
	if(count($explode)>1){
		$name['fname'] = $explode[0];
		$name['initials'] = $explode[1];
	...
	private function split_full_name($full_name) {
	    ...
	    // split into words
	    $unfiltered_name_part = explode(" ",$full_name);

	    foreach ($unfiltered_name_part as $key => $value) {
	    	if(!empty($value)){
	    		$name_parts[]=$value;
	    	}
	    }
	...
	private function is_salutation($word) {
	    // ignore periods
	...
	private function is_compound_lname($word) {
	...
	private function is_initial($word) {
	...
	private function safe_ucfirst($seperator, $word) {
